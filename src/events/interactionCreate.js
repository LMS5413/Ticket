...
module.exports = {
    async execute(client, interaction) {
        ...
        const ticket = await dbTicket.findOne({ where: { idc: interaction.channel.id } }) || await dbTicket.findOne({ where: { id: interaction.user.id } });
        ...
        const category = (await departaments.findAll()).filter(x => x.getDataValue('id_guild') === interaction.guild.id).find(x => x.getDataValue('name').toLowerCase().replace(/([\u2700-\u27BF]|[\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2011-\u26FF]|\uD83E[\uDD10-\uDDFF])/g,'').replace(/\s+/g, ' ').trim().split("-")[0] === interaction.values[0].split("-")[0])
